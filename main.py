...
class connected_label(object):
    # class creates a connection status label with connected or not connected states
    def __init__(self):
...
class connect_to_switch(object):
    ...
    def get_login_credentials(self):
        ...
        self.ip_addr = ip_addr_entry.get()
...
class monitor_sessions(object):
    ...
    def refresh_list(self):
        ...
        output_text.insert(END, 'Current port mirroring configuration:')
...
class new_monitor_session(object):
    ...
    def __init__(self):
        ...
        self.new_session_name = user_session_name.get()
        self.source_port_number = user_source_port.get()
        self.destination_port_number = user_destination_port.get()
...
class new_automated_session(object):
    ...
    def initiate(self):
        ...
        self.index = 0
        self.new_session_name = user_session_name_automate.get()
        self.first_source_port_number = int(start_user_source_port.get())
        self.last_source_port_number = int(end_user_source_port.get())
        self.destination_port_number = user_destination_port_automate.get()
        self.port_range = []
...
# The tool uses only one window
# Window is created, title and icon are assigned


main_window = Tk()
main_window.title('IKH_eAPI')

# Uncomment if the icon file is placed near .py file for compiling into .exe
# And add the actual path to the icon!

# main_window.iconbitmap(%PATH_TO_ICON%\icon.ico')
...
login_first_label = Label(
    main_window, text='Login First', font='Arial 12 bold')
login_first_label.grid(row=1, columnspan=3)

# IP address label and input text field

ip_addr_label = Label(main_window, text='Device IP address', font='Arial 10')
ip_addr_label.grid(row=3, column=0, sticky=E)

ip_addr_entry = Entry(main_window, font='Arial 10')
ip_addr_entry.grid(row=3, column=1)

# Username label and input text field

username_label = Label(main_window, text='Username', font='Arial 10')
username_label.grid(row=4, column=0, sticky=E)

username_entry = Entry(main_window, font='Arial 10')
username_entry.grid(row=4, column=1)

# Password label and input text field, password is hidden when entering

password_label = Label(main_window, text='Password', font='Arial 10')
password_label.grid(row=5, column=0, sticky=E)

password_entry = Entry(main_window, show='*', font='Arial 10')
...
login_button = Button(main_window, text='Login', width=30,
                      command=switch_login, font='Arial 10')
login_button.grid(row=6, columnspan=3)

# Dummy field. Replaced by login status when connection is active

status_label = Label(main_window, text='Connection Status', font='Arial 10')
...
new_session_label = Label(
    main_window, text='Create or modify session', font='Arial 12 bold')
new_session_label.grid(row=10, columnspan=3)

# Session name label and text field

session_name_label = Label(main_window, text='Session name', font='Arial 10')
session_name_label.grid(row=11, column=0, sticky=E)

user_session_name = Entry(main_window, font='Arial 10')
user_session_name.grid(row=11, column=1)

# Source port label and text field

source_port_label = Label(main_window, text='Source port', font='Arial 10')
source_port_label.grid(row=12, column=0, sticky=E)

user_source_port = Entry(main_window, font='Arial 10')
user_source_port.grid(row=12, column=1)

# Destination port label and text field

destination_port_label = Label(
    main_window, text='Destination port', font='Arial 10')
destination_port_label.grid(row=13, column=0, sticky=E)


user_destination_port = Entry(main_window, font='Arial 10')
user_destination_port.grid(row=13, column=1)

# TX only button. Calls for TX only new session function

tx_only_button = Button(main_window, text='Source port: TX only',
                        width=30, command=tx_session_creation, font='Arial 10')
tx_only_button.grid(row=14, columnspan=3)

# RX only button. Calls for RX only new session function

rx_only_button = Button(main_window, text='Source port: RX only',
                        width=30, command=rx_session_creation, font='Arial 10')
rx_only_button.grid(row=15, columnspan=3)

# Duplex button. Calls for duplex new session function

duplex_button = Button(main_window, text='Source port: Duplex',
                       width=30, command=duplex_session_creation, font='Arial 10')
duplex_button.grid(row=16, columnspan=3)
...
remove_session_label = Label(
    main_window, text='Remove session', font='Arial 12 bold')
remove_session_label.grid(row=17, columnspan=3)

# Session to remove name text field and title

session_remove_name_label = Label(
    main_window, text='Session name', font='Arial 10')
session_remove_name_label.grid(row=19, column=0, sticky=E)

remove_session_name = Entry(main_window, font='Arial 10')
remove_session_name.grid(row=19, column=1)

# Session remove button, calls session remove function

remove_session_button = Button(main_window, text='Remove session',
                               width=30, command=kill_monitor_session, font='Arial 10')
remove_session_button.grid(row=20, columnspan=3)
...
list_label = Label(main_window, text='Active sessions', font='Arial 12 bold')
list_label.grid(row=21, columnspan=3)

# Session refresh button. Calls session list refresh function

sessions_list_button = Button(main_window, text='Refresh sessions list',
                              width=30, command=monitor_sessions_list, font='Arial 10')
sessions_list_button.grid(row=22, columnspan=3)
...
list_label = Label(main_window, text='Automated switching (Duplex only)',
                   font='Arial 12 bold')
list_label.grid(row=23, columnspan=3)

# Session name label and text field

session_name_label_automate = Label(
    main_window, text='Session name', font='Arial 10')
session_name_label_automate.grid(row=24, column=0, sticky=E)

user_session_name_automate = Entry(main_window, font='Arial 10')
user_session_name_automate.grid(row=24, column=1)

# Start source port label and text field

start_source_port_label = Label(
    main_window, text='First source port', font='Arial 10')
start_source_port_label.grid(row=25, column=0, sticky=E)

start_user_source_port = Entry(main_window, font='Arial 10')
start_user_source_port.grid(row=25, column=1)


# End source port label and text field

end_source_port_label = Label(
    main_window, text='Last source port', font='Arial 10')
end_source_port_label.grid(row=26, column=0, sticky=E)

end_user_source_port = Entry(main_window, font='Arial 10')
end_user_source_port.grid(row=26, column=1)


# Destination port label and text field

destination_port_label_automate = Label(
    main_window, text='Destination port', font='Arial 10')
destination_port_label_automate.grid(row=27, column=0, sticky=E)


user_destination_port_automate = Entry(main_window, font='Arial 10')
user_destination_port_automate.grid(row=27, column=1)


# Start automation button, initiates port-by-port monitoring sessions

duplex_only_button = Button(main_window, text='Start',
                            width=30, command=start_duplex_automation, font='Arial 10')
duplex_only_button.grid(row=29, columnspan=3)


# Next port button

next_button = Button(main_window, text='Next port',
                     width=30, command=next_automated, font='Arial 10')
next_button.grid(row=31, padx=1, columnspan=3)
