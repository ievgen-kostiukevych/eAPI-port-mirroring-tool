...
class monitor_sessions(object):
    ...
    def refresh_list(self):
        ...
        main_app.output_text.insert(
            tk.END, 'Current port mirroring configuration:')
...
class new_automated_session(object):
    ...
    def initiate(self):
        ...
        self.port_range = []
        for port in self.list_of_ports.split(','):
...
def monitor_sessions_list():
    ...
    if main_app.switch_is_connected:
...
class MainApplication(tk.Frame):
    ...
    def login_section(self):
        # Section title
        self.login_first_label = tk.Label(
            self.master, text='Login First', font='Arial 12 bold')
        self.login_first_label.grid(row=1, columnspan=3)

        # IP address label and input text field
        self.ip_addr_label = tk.Label(
            self.master, text='Device IP address', font='Arial 10')
        ...
        self.username_label = tk.Label(
            self.master, text='Username', font='Arial 10')
        ...
        self.password_label = tk.Label(
            self.master, text='Password', font='Arial 10')
        ...
        self.status_label = tk.Label(
            self.master, text='Connection Status', font='Arial 10')
        self.status_label.grid(row=7, columnspan=3)

    def connected(self):
        # Basic switch info is displayed when connected
        self.status_bar = tk.Label(self.master, bg='green', font='Arial 10', text='Connected to: {} at {}'.format(
            switch.switch_hostname['result'][0]['hostname'], switch.ip_addr), bd=1, relief=tk.SUNKEN, anchor=tk.CENTER)
        self.switch_is_connected = True

    def not_connected(self):
        # if not connected - error displayed
        self.status_bar = tk.Label(self.master, bg='red', font='Arial 10',
                                   text='Connection error, check parameters!', bd=1, relief=tk.SUNKEN, anchor=tk.CENTER)
        self.switch_is_connected = False
    ...
    def display_label(self):
        ...
        try:
            ...
            self.status_bar.grid(row=7, columnspan=5,
                                 sticky=tk.N+tk.S+tk.E+tk.W)

    def new_section_section(self):
        # Section title
        self.new_session_label = tk.Label(
            self.master, text='Create or modify session', font='Arial 12 bold')
        self.new_session_label.grid(row=10, columnspan=3)

        # Session name label and text field
        self.session_name_label = tk.Label(
            self.master, text='Session name', font='Arial 10')
        ...
        self.source_port_label = tk.Label(
            self.master, text='Source port', font='Arial 10')
        ...
        self.destination_port_label = tk.Label(
            self.master, text='Destination port', font='Arial 10')
        ...
        self.tx_only_button = tk.Button(
            self.master, text='Source port: TX only', width=30, command=tx_session_creation, font='Arial 10')
        self.tx_only_button.grid(row=14, columnspan=3)

        # RX only button. Calls for RX only new session function
        self.rx_only_button = tk.Button(
            self.master, text='Source port: RX only', width=30, command=rx_session_creation, font='Arial 10')
        self.rx_only_button.grid(row=15, columnspan=3)

        # Duplex button. Calls for duplex new session function
        self.duplex_button = tk.Button(
            self.master, text='Source port: Duplex', width=30, command=duplex_session_creation, font='Arial 10')
        self.duplex_button.grid(row=16, columnspan=3)

    def remove_session_section(self):

        # Section title
        self.remove_session_label = tk.Label(
            self.master, text='Remove session', font='Arial 12 bold')
        self.remove_session_label.grid(row=17, columnspan=3)

        # Session to remove name text field and title
        self.session_remove_name_label = tk.Label(
            self.master, text='Session name', font='Arial 10')
        ...
        self.remove_session_button = tk.Button(
            self.master, text='Remove session', width=30, command=kill_monitor_session, font='Arial 10')
    ...
    def session_list_section(self):
        ...
        self.list_label = tk.Label(
            self.master, text='Active sessions', font='Arial 12 bold')
        self.list_label.grid(row=21, columnspan=3)

        # Session refresh button. Calls session list refresh function
        self.sessions_list_button = tk.Button(
            self.master, text='Refresh sessions list', width=30, command=monitor_sessions_list, font='Arial 10')
...
""")
    ...
    def automated_switching(self):
        ...
        # Section title
        self.list_label = tk.Label(
            self.master, text='Automated switching (Duplex only)', font='Arial 12 bold')
        self.list_label.grid(row=23, columnspan=3)

        # Session name label and text field
        self.session_name_label_automate = tk.Label(
            self.master, text='Session name', font='Arial 10')
        self.session_name_label_automate.grid(row=24, column=0, sticky=tk.E)

        self.user_session_name_automate = tk.Entry(
            self.master, font='Arial 10')
        self.user_session_name_automate.grid(row=24, column=1)

        # Start source port label and text field
        self.start_source_port_label = tk.Label(
            self.master, text='List of source ports', font='Arial 10')
        self.start_source_port_label.grid(row=25, column=0, sticky=tk.E)

        self.user_list_of_ports = tk.Entry(self.master, font='Arial 10')
        self.user_list_of_ports.grid(row=25, column=1)

        # Destination port label and text field
        self.destination_port_label_automate = tk.Label(
            self.master, text='Destination port', font='Arial 10')
        self.destination_port_label_automate.grid(
            row=27, column=0, sticky=tk.E)

        self.user_destination_port_automate = tk.Entry(
            self.master, font='Arial 10')
        ...
        self.duplex_only_button = tk.Button(
            self.master, text='Start', width=30, command=start_duplex_automation, font='Arial 10')
        self.duplex_only_button.grid(row=29, columnspan=3)

        # Next port button
        self.next_button = tk.Button(
            self.master, text='Next port', width=30, command=next_automated, font='Arial 10')
